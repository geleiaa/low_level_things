...
int main(int argc, char *argv[]) {
    ...
    NTSTATUS retval = NtCreateSection(&sectionHandle, sectionFlags, NULL, (PLARGE_INTEGER)&sectionSize, PAGE_EXECUTE_READWRITE, SEC_COMMIT, NULL);
    if (retval != 0) {
        printf("NtError %lx\n", retval);
        return 1;
    }
    ...
    retval = NtMapViewOfSection(sectionHandle, (HANDLE)-1, &localSectionAddress, 0, 0, 0, &size, 2, 0, PAGE_READWRITE);
    if (retval != 0) {
        printf("NtError %lx\n", retval);
        return 1;
    }
    ...
    retval = NtMapViewOfSection(sectionHandle, targetHandle, &remoteSectionAddress, 0, 0, 0, &size, 2, 0, PAGE_EXECUTE_READ);
    if (retval != 0) {
        printf("NtError %lx\n", retval);
        return 1;
    }
